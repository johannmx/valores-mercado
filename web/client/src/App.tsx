  ...
  return (
    <div className="space-y-8 bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-100 dark:border-slate-700/50">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-slate-100 dark:border-slate-700/50">
        {['USD', 'ARS_BLUE', 'VES', 'EUR'].map(key => (
          <div key={key} className="bg-white dark:bg-slate-700/30 p-4 rounded-xl border border-slate-100 dark:border-slate-600/50">
            <p className="text-[9px] font-black text-slate-400 uppercase">{key}</p>
            <p className="font-black text-slate-800 dark:text-white">${convertUsdTo(key as any)}</p>
          </div>
        ))}
      </div>
    </div>
