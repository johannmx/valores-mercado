...
## 2024-05-04 - [HIGH] HTTP Parameter Injection via curl in Bash
**Vulnerability:** The bash script `dolar_telegram_notification.sh` made a POST request to Telegram's API using `curl` with `-d "text=${MSG}"`. If the `MSG` variable contained special characters such as `&` or `=`, they could have been interpreted as parameter delimiters rather than literal text. This could allow an attacker to inject new parameters or modify existing ones within the POST request.
**Learning:** Bash variables are substituted literally. When constructing a payload for a web request, relying on bash string interpolation with basic `-d` flags leaves the script vulnerable to parameter injection if the text contains URL-encoded form delimiters.
**Prevention:** Always use `--data-urlencode` when passing variables that might contain dynamic or multiline text to `curl` instead of the basic `-d`/`--data` flags. This instructs `curl` to properly percent-encode the payload, ensuring it is transmitted as the intended value for the parameter rather than structural data.
