...
## 2026-07-13 - [HIGH] Missing CORP and COOP Security Headers
**Vulnerability:** The Fastify backend was missing `Cross-Origin-Resource-Policy` (CORP) and `Cross-Origin-Opener-Policy` (COOP) headers. This made the application slightly more vulnerable to XS-Leaks and Spectre-class attacks.
**Learning:** By default, Helmet may omit certain modern headers or configure them for strict same-origin only, which breaks cross-origin frontend requests.
**Prevention:** Always configure `@fastify/helmet` explicitly with `crossOriginResourcePolicy: { policy: 'cross-origin' }` and `crossOriginOpenerPolicy: { policy: 'same-origin' }` when serving an API accessed by different frontend origins.
