...
## 2026-04-25 - [HIGH] Server-Side Request Forgery (SSRF) via Upstream APIs
**Vulnerability:** The Fastify backend fetches data from various external APIs using Axios without disabling HTTP redirects. A compromised or malicious upstream API could respond with a redirect (e.g., 301, 302) to an internal service or a metadata endpoint (like `169.254.169.254` on AWS), causing the backend to unknowingly execute a Server-Side Request Forgery (SSRF) attack.
**Learning:** By default, HTTP clients like Axios will automatically follow redirects. When fetching data from third-party services, this default behavior introduces a significant SSRF risk, as the application assumes the response comes from the original, trusted destination, but it may actually be retrieving sensitive internal data or interacting with internal endpoints.
**Prevention:** Always disable automatic HTTP redirects globally when interacting with external APIs unless specifically required. Set `axios.defaults.maxRedirects = 0;` to prevent the application from blindly following potentially malicious redirects.
